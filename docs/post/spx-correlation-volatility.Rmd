...
h <- xml2::read_html("https://en.wikipedia.org/wiki/List_of_S&P_500_companies")
syms <- xml2::xml_find_all(h, "//table[@id='constituents']//tr/td[1]")
syms <- xml2::xml_text(syms)
syms <- sub("\n", "", syms, fixed = TRUE)
syms <- sub(".", "-", syms, fixed = TRUE)

e <- new.env()
getSymbols(syms, src = 'tiingo', api.key = tiingo.key, env = e)
d <- do.call(merge, lapply(e, Cl))
colnames(d) <- sub(".Close", "", colnames(d), fixed = TRUE)
...
cors <- rollapply(returns, 60, mean_cor, by.column = FALSE, align = "right", na.rm = TRUE)
```
