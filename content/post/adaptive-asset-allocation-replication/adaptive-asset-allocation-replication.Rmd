...
date: 2023-08-31T13:04:00.000-05:00
draft: false
categories: [ Articles ]
tags: [R]
urlcolor: blue
...
```{r data, include=FALSE, purl=TRUE}
splice_returns <-
function(Ra, Rb)
{
...
The original paper showed results as a monthly series. The `to_monthly_returns()` function converts the strategy returns from daily to monthly. The `strat_summary()` function uses the [PerformanceAnalytics](https://cran.r-project.org/package=PerformanceAnalytics) package to calculate summary statistics to compare with the original paper.
...
The file [adaptive-asset-allocation-replication.R](adaptive-asset-allocation-replication.R) contains all the code used in this analysis.
...
The `estimate_equal_risk_portf()` function estimates the equal risk contribution portfolio using the `PERC()` function from the [FRAPO](https://cran.r-project.org/package=FRAPO) package. Setting `percentage = FALSE` returns the weights as decimals instead of percents (i.e. 0.2 instead of 20 for 20% weight).

The `portf_equal_risk()` function calculates the portfolio weights at the end of month using estimated portfolio risk from the returns over the last 60 days. Then those weights are used to calculate the portfolio returns for the following month.
\
<!-- new line for spacing -->
...
codetools::checkUsage(estimate_equal_risk_portf, all = TRUE)
codetools::checkUsage(portf_equal_risk, all = TRUE)
...
codetools::checkUsage(portf_top_momentum, all = TRUE)
...
```{r top_momo_eq_risk_2, echo=FALSE}
...
codetools::checkUsage(portf_top_momentum_equal_risk, all = TRUE)
...
codetools::checkUsage(estimate_min_var_portf, all = TRUE)
codetools::checkUsage(portf_top_momentum_min_var, all = TRUE)
...
```{r top_momo_min_var_2, echo=FALSE}
portf_top_momentum_min_var_2 <-
function(returns,
         n_assets = 5,
         n_days = 120,
         n_days_vol = 60,
...
# Extension
-->
```{r top_momo_min_var_not_always_invested, eval=FALSE, echo=FALSE, purl=FALSE}
